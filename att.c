...
static u8_t att_handle_rsp(struct bt_att *att, void *pdu, u16_t len, u8_t err)
{
	...
	if (att->req->buf) {
		printf("Release original buffer\n");
...
struct net_buf *bt_att_create_pdu(struct bt_conn *conn, u8_t op, size_t len)
{
	...
	// if(len ==0)
	// 	return NULL;
	printf("bt_att_create_pdu >>>mtu = %u   size of len: %u\r\n",att->chan.tx.mtu,len);
...
int bt_att_req_send(struct bt_conn *conn, struct bt_att_req *req)
{
	...
	if (att->req) {
		...
		printf("Queue the request to be send later");
